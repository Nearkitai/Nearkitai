...
int nAnonymizeNearkitaiAmount = 1000;
...
#else
    const char* pszModule = "nearkitai";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Nearkitai
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Nearkitai
    // Mac: ~/Library/Application Support/Nearkitai
    // Unix: ~/.nearkitai
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Nearkitai";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nearkitai.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nearkitaid.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong NEARKITAI will not work properly.");
