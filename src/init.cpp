...
void Shutdown()
{
    ...
    RenameThread("nearkitai-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: nearkitai.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: nearkitaid.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 12040)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 12041)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizenearkitaiamount=<n> " + _("Keep N Nearkitai anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Nearkitai is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Nearkitai is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Nearkitai version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Nearkitai server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Nearkitai") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Nearkitai to complete") << "\n";
    ...
    nAnonymizeNearkitaiAmount = GetArg("-anonymizenearkitaiamount", 0);
    if(nAnonymizeNearkitaiAmount > 999999) nAnonymizeNearkitaiAmount = 999999;
    if(nAnonymizeNearkitaiAmount < 2) nAnonymizeNearkitaiAmount = 2;
    ...
    LogPrintf("Anonymize Nearkitai Amount %d\n", nAnonymizeNearkitaiAmount);
