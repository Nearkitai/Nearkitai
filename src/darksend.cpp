    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Nearkitai and make it unusable. To
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeNearkitaiAmount*COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nDarksendRounds))
    {
        ...
        if (pwalletMain->SelectCoinsDark(nValueMin, 244999*COIN, vCoins, nValueIn, -2, 0))
        {
...
bool CDarksendPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, nChangePos, strFail, coinControl, ONLY_NONDENOMINATED_NOT3000IFMN);
    if(!success){
        ...
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
                nFeeRet, nChangePos, strFail, coinControl, ONLY_NOT3000IFMN);
        if(!success){
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("nearkitai-darksend");
