...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeNearkitaiAmount"))
        settings.setValue("nAnonymizeNearkitaiAmount", 1000);
    nAnonymizeNearkitaiAmount = settings.value("nAnonymizeNearkitaiAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeNearkitaiAmount"))
        SoftSetArg("-anonymizenearkitaiamount", settings.value("nAnonymizeNearkitaiAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeNearkitaiAmount:
            return QVariant(nAnonymizeNearkitaiAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeNearkitaiAmount:
            nAnonymizeNearkitaiAmount = value.toInt();
            settings.setValue("nAnonymizeNearkitaiAmount", nAnonymizeNearkitaiAmount);
            emit AnonymizeNearkitaiAmountChanged(nAnonymizeNearkitaiAmount);
            break;
        default:
            break;
        }
