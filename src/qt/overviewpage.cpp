...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeNearkitaiAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeNearkitaiAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeNearkitaiAmount = strAnonymizeNearkitaiAmount.remove(strAnonymizeNearkitaiAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNearkitaiAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeNearkitaiAmount*COIN) nMaxToAnonymize = nAnonymizeNearkitaiAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeNearkitaiAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeNearkitaiAmount));
        strAnonymizeNearkitaiAmount = strAnonymizeNearkitaiAmount.remove(strAnonymizeNearkitaiAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeNearkitaiAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeNearkitaiAmount == 0){
