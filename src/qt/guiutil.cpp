...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("nearkitai"))
        return false;
...
{
    // Convert nearkitai:// to nearkitai:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("nearkitai://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "nearkitai:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Nearkitai.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Nearkitai.lnk
...
{
    return GetAutostartDir() / "nearkitai.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a nearkitai.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Nearkitai\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the nearkitai app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add nearkitai app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Nearkitai-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  nearkitai-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Nearkitai-Qt"));
